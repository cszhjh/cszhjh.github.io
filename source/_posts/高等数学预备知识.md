...
见到 $\cfrac{1}{u} (u>0)$, 用 $u$ 研究即可

见到 $u_1u_2\cdots u_n=u$, 用 $\ln{u}=\sum_\limits{i=1}^{n}{\ln{u_i}}$ 研究即可

### 三角函数和反三角函数 ⭐

性质: $\arcsin x+\arccos x=\cfrac{\pi}{2}(-1\leq x\leq 1)$

性质: $\arctan x + \arccot x=\cfrac{\pi}{2} (-\infty < x < +\infty)$
...
1. 当 **$k>1$** 时的图像, 可由 $y=f(x)$ 的图像上每点的横坐标都**缩短**到原来的 **$\cfrac{1}{k}$** 倍, 且纵坐标不变
2. 当 $0 < k < 1$ 时的图像, 可由 $y=f(x)$ 的图像上每点的横坐标**伸长**到原来 **$\cfrac{1}{k}$** 倍, 且纵坐标不变
...
#### 前 n 项和
$$
S_n=\cfrac{n}{2}[2a_1+(n-1)d]=\cfrac{n}{2}(a_1+a_n)
$$
...
$$
\sum_{k=1}^n k=1+2+3+...+n=\cfrac{n(n+1)}{2}
$$

$$
\sum_\limits{k=1}^n k^2=1^2+2^2+3^2+...+n^2=\cfrac{n(n+1)(2n+1)}{6}
$$

$$
\sum_\limits{k=1}^n \cfrac{1}{k(k+1)}=\cfrac{1}{1\times 2}+\cfrac{1}{2\times 3}+\cfrac{1}{3\times 4}+...+\cfrac{1}{n\times (n+1)}=\cfrac{n}{n\times (n+1)}
$$
...
$\cot2\alpha=\cfrac{\cot^2\alpha-1}{2\cot\alpha}$
...
$cot(\alpha\pm\beta)=\cfrac{\cot\alpha\cot\beta\mp1}{\cot\alpha\pm\cot\beta}$

## 指数运算法则 ⭐

$x^\alpha\ast x^\beta=x^{\alpha+\beta}$

$\cfrac{x^\alpha}{x^\beta}=x^{\alpha-\beta}$

$(x^\alpha)^\beta=x^{\alpha\beta}$

$(xy)^\alpha=x^\alpha y^\alpha$

$(\cfrac{x}{y})^\alpha=\cfrac{x^\alpha}{y^\alpha}$
...
$log_a \sqrt[n]{M}=\cfrac{1}{n}\log_a M$

## 一元二次方程基础

根的公式: $x_{1,2}=\cfrac{-b\pm \sqrt{b^2-4ac}}{2a}$
...
抛物线: $y=ax^2+bx+c$ 的顶点 $(-\cfrac{b}{2a}, c-\cfrac{b^2}{4a})$
...
$$
\sqrt {ab}\leq \cfrac{a+b}{2}\leq \sqrt {\cfrac{a^2+b^2}{2}}(a,b>0)
$$

$$
\sqrt {abc}\leq \cfrac{a+b+c}{3}\leq \sqrt {\cfrac{a^2+b^2+c^2}{3}}(a,b,c>0)
$$
...
0<c<y<d
\rbrace
\Rightarrow \cfrac{c}{b}<\cfrac{y}{x}<\cfrac{d}{a}
$$
...
$$
\cfrac{1}{1+x}<\ln(1+\frac{1}{x})<\cfrac{1}{x}(x>0)
$$
