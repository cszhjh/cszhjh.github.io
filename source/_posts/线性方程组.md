...
1. $(2)\text{唯一解}\Leftrightarrow D\not=0\Leftrightarrow r(A)=n\Leftrightarrow r(A)=r(\bar{A})=n$
2. $x_1=\frac{D_1}{D},\cdots,x_n=\frac{D_n}{D}$
3. $D=0\Leftrightarrow r(A)<n\begin{cases} r(A)=r(\bar{A})<n, & \text{无数解} \\ r(A)\not=r(\bar{A}), & \text{无解} \end{cases}$
